"""
    ...
    calculates anomaly using z-score method and store results to InfluxDB
"""
...
def on_message(mqttclient, userdata, message):
    ...
    try:
        ...
        z_score, \
        z_score_thresh \
         = time_type_analytics(time_type, time_value)

        # Store results into InfluxDB
        try:
            ...
            point = (
                ...
                .field("z_score", round(float(z_score), 4))
...
if __name__ == "__main__":
    ...
    air_valve_extend_cmd = AnomalyDetectionZscore("AirValveExtendCmd",
                                                  MODEL_WINDOW_SIZE,
                                                  ANOMALY_LIST_SIZE,
                                                  logger)
    air_valve_extend = AnomalyDetectionZscore("AirValveExtend",
                                              MODEL_WINDOW_SIZE,
                                              ANOMALY_LIST_SIZE,
                                                  logger)
    air_valve_retract_cmd = AnomalyDetectionZscore("AirValveRetractCmd",
                                                   MODEL_WINDOW_SIZE,
                                                   ANOMALY_LIST_SIZE,
                                                   logger)
    air_valve_retract = AnomalyDetectionZscore("AirValveRetract",
                                               MODEL_WINDOW_SIZE,
                                               ANOMALY_LIST_SIZE,
                                               logger)
    ...
    try:
        ...
        logger.error("Setting up of MQTT failed. Error code/reason: %s", e)
